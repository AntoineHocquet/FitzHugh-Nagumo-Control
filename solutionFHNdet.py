...
def FHNdet(wgate,ic,dt,T,N,a,b,c,I,sigex,J,sigJ,Vrev,ar,ad,Tmax,la,VT):
    ...
    #mean vector for initial distribution
    m=np.zeros([3])
    ...
    #cov matrix for initial distribution
    cov=np.zeros([3,3])
    ...
    for r in range(1,M):   
        ...
        for i in range(N):
            ...
            def drift(y):
                ...
                f=np.zeros([3,1])      
                f[0,0]=y[0]-(np.power(y[0],3)/float(3))-y[1]+I[0,r-1]
                f[1,0]=c*(y[0]+a-b*y[1])
            ...
            def funMin(y):
                ...
                func=y-((z[i][:,r-1]).reshape([3,1])+drift(y)+np.sqrt(dt)*g.dot(w)+np.sqrt(dt)*u*h).reshape([3])
